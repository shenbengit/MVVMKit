 ...
 */
fun <T> BaseSupportFragment<*, *>.httpRequest(
...
) = viewLifecycleOwner.lifecycleScope.httpRequest(block, {
 ...
 * IO请求
 */
fun <T> BaseSupportFragment<*, *>.request(
    block: suspend CoroutineScope.() -> T,
    onSuccess: (ResultStatus.Success<T>) -> Unit = {},
    onError: (ResultStatus.Error) -> Unit = {},
    onComplete: () -> Unit = {},
    isShowLoadingDialog: Boolean = true
) = viewLifecycleOwner.lifecycleScope.request(block, {
    if (isShowLoadingDialog) {
        showLoadingDialog()
    }
}, onSuccess, onError, {
